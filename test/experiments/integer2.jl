...
a = CSV.read("test/experiments/glass.csv")
df = DataFrame(CSV.File("test/experiments/glass.csv"))
seed = 1
...
function generateIndividual(rng)
    ...
    # n_subfeatures
    ind[4] = rand(rng, [1,2,3,4,5,6,7,8,9])
...
function mutateTree(genotype, rng)
    ...
    if rand(rng) < 0.2
        ...
        elseif index == 4
            copyRep[4] = rand(rng, [1,2,3,4,5,6,7,8,9])
        end
    end
...
val, time, bytes, gctime, memallocs = @timed runExperiment(verbose=false)
#@time runExperiment(verbose=false)

println("Tiempo: ",time, " Memoria KB: ", bytes/1024/1024)
...
plot(y[1:500], x[1,1:500], titlefont=myfont, tickfont=myfont,
          legendfont=myfont, yguide="Valores de calidades", xguide="Generaciones",
          guidefont=myfont, label="Media", lw=2, ylims=[0.6,1])



plot!(y[1:500], x2[1,1:500], label="Mejor individuo")


savefig("Integer3_$seed.png")
